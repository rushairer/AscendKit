...
```bash
scripts/install-ascendkit.sh --version 1.10.0
...
```bash
scripts/verify-release-assets.sh --version 1.10.0
```
...
scripts/verify-homebrew-formula.sh --version 1.10.0
scripts/diagnose-homebrew-install.sh --version 1.10.0
scripts/sync-homebrew-tap.sh --commit --push
scripts/finalize-homebrew-release.sh --version 1.10.0 --commit --push --reinstall
scripts/v1-release-readiness.sh --version 1.10.0 --app-root /path/to/RepresentativeApp
...
List and idempotently synchronize Game Center Achievement definitions through the App Store Connect API v2 achievement resources.

```bash
ascendkit asc achievements list \
  --workspace "$WORKSPACE" \
  --group-id "$GAME_CENTER_GROUP_ID" \
  --json

ascendkit asc achievements sync \
  --workspace "$WORKSPACE" \
  --group-id "$GAME_CENTER_GROUP_ID" \
  --file docs/game-center-achievements.example.json \
  --json

ascendkit asc achievements sync \
  --workspace "$WORKSPACE" \
  --group-id "$GAME_CENTER_GROUP_ID" \
  --file /path/to/achievements.json \
  --confirm-remote-mutation \
  --json
```

Use `--detail-id` instead of `--group-id` for a standalone Game Center detail. The catalog key is `vendorIdentifier`: existing achievements are updated, missing achievements are created, and unchanged values are skipped. Localizations are synchronized per locale. The command defaults to a dry run; remote changes require `--confirm-remote-mutation`. Start from `docs/game-center-achievements.example.json`.
