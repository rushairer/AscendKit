...
struct CLISmokeTests {
    ...
    func reportsCurrentSemanticVersion() {
        #expect(AscendKitVersion.current == "1.10.0")
    }
    ...
    func agentDocsMentionRequiredHandoffCommands() throws {
        ...
        #expect(playbook.contains("--workspace \"$WORKSPACE\""))
        ...
        #expect(readme.contains("--workspace \"$WORKSPACE\""))
    ...
    func releasePackagingScriptAndDocsStayDiscoverable() throws {
        ...
        #expect(representativeSmokeScript.contains("agent prompt"))
        #expect(representativeSmokeScript.contains("--workspace \"${WORKSPACE}\""))
        #expect(representativeSmokeScript.contains("representative-smoke-profile"))
        ...
        #expect(readme.contains("### `agent`"))
        #expect(readme.contains("ascendkit agent prompt \\"))
        #expect(readme.contains("Use `--workspace` when a release workspace already exists"))
        #expect(readme.contains("`agent prompt` refuses placeholder-style sample values"))
