...
For agent handoff, share the `workspace export-summary` JSON instead of zipping or copying `.ascendkit/`. The export is intentionally status-only, includes `ascendKitVersion`, and excludes raw release artifacts.
